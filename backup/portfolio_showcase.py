...
with col1:
    ...
    st.multiselect(
        ...
        on_change=lambda: st.session_state.update({"country_sel": []})
    )
    ...
    if st.session_state.get("last_regions") != st.session_state.region_sel:
        st.session_state.last_regions = st.session_state.region_sel
        st.rerun()
...
if st.button("Reset All Filters"):
    for key in ["registry_sel", "region_sel", "country_sel", "scope_sel", "type_sel", "redrem_sel"]:
...
# ============== PROJECTS BY SCOPE (2-COLUMN LAYOUT) ==============
st.markdown(
    "<h4 style='text-align: center; font-size:30px;'>Projects by Scope (Side by Side)</h4>", 
    unsafe_allow_html=True
)
...
with col1:
    st.markdown(
        "<h5 style='text-align: center; font-size:20px;'>Projects by Scope</h5>", 
        unsafe_allow_html=True
    )
    ...
    if (df_sel.empty) or (not needed_cols) or (not needed_cols.issubset(df_sel.columns)):
        ...
        fig_sb.update_layout(
            margin=dict(t=20, r=10, b=20, l=10)
        )
...
with col2:
    st.markdown(
        "<h5 style='text-align: center; font-size:20px;'>Projects by Scope by Standard</h5>", 
        unsafe_allow_html=True
    )
