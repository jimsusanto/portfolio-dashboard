...
st.markdown("""
<style>
    /* Reduce spacing for the filter wrapper */
    .filter-item {
        margin-bottom: 0.05rem !important;
    }

    /* Correct selector for Streamlit multiselect widgets */
    div[data-testid="stMultiSelect"] {
        margin-bottom: 0.05rem !important;  /* even smaller spacing between rows */
    }
</style>
...
# Title
st.markdown(
    "<h1 style='margin-top:0; margin-bottom:0;'>Carbon Project Analysis Dashboard</h1>", 
    unsafe_allow_html=True
)
...
def safe_update_session_state(key, value):
    """Safely update session state only if the widget hasn't been created yet"""
    # Only update if the key doesn't exist or the value is different
    if key not in st.session_state or st.session_state.get(key) != value:
        st.session_state[key] = value

def reset_session_state_safely():
    """Safely reset session state for all filter keys"""
    # Store the current widget states to avoid StreamlitAPIException
    widget_states = {}
    
    # For each filter key, we'll set it to its default value
    # but only if it doesn't conflict with existing widgets
    reset_values = {
        "registry_sel": [],
        "region_sel": [],
        "country_sel": [],
    ...
    # Only update keys that exist in session state
    for key, value in reset_values.items():
        if key in st.session_state:
            # Check if the current value is different from the reset value
            if st.session_state[key] != value:
                # Use direct assignment only for safe cases
                try:
                    st.session_state[key] = value
                except StreamlitAPIException:
                    # If we get an exception, we'll let the rerun handle the reset
                    pass
...
# ========= 1) compute ALL options FIRST (unfiltered) =========
...
def _apply_master_all():
    ...
    if st.session_state.master_all:
        ...
        safe_update_session_state("region_sel", region_opts[:])
        safe_update_session_state("scope_sel", scope_opts[:])
        safe_update_session_state("registry_sel", registry_opts[:])
        safe_update_session_state("redrem_sel", redrem_all[:])   # provisional; may reduce after child opts computed

def _clear_type_on_scope_change():
    """Only clear Type when Scope changes (not when Region changes)"""
    safe_update_session_state("type_sel", [])

# Compute type options before creating filters
...
# Create 2x3 grid layout for filters
col1, col2, col3 = st.columns(3)

with col1:
    # Region and Country in first column
    st.markdown('<div class="filter-item" style="margin-bottom: 0.05rem;">', unsafe_allow_html=True)
    def clear_country_selection():
        safe_update_session_state("country_sel", [])
    ...
    st.multiselect(
        ...
        on_change=clear_country_selection
    )
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="filter-item" style="margin-bottom: 0.05rem;">', unsafe_allow_html=True)
    # Force rerun to update country options when regions change
    if st.session_state.get("last_regions") != st.session_state.region_sel:
        safe_update_session_state("last_regions", st.session_state.region_sel)
        st.rerun()
    
    # Get filtered countries based on selected regions
    filtered_countries = country_opts
    if st.session_state.region_sel:
        filtered_df = df_projects[df_projects["Region"].isin(st.session_state.region_sel)]
        filtered_countries = uniq_sorted(filtered_df["Country"]) if "Country" in filtered_df else []
    
    # Get current selection from session state (filtered to available options)
    current_selection = [v for v in st.session_state.country_sel if v in filtered_countries]
...
with col2:
    # Scope and Type in second column
    st.markdown('<div class="filter-item" style="margin-bottom: 0.05rem;">', unsafe_allow_html=True)
    st.multiselect(
        "Scope",
        options=scope_opts,
    ...
    st.markdown('<div class="filter-item" style="margin-bottom: 0.05rem;">', unsafe_allow_html=True)
    # Get current selection from session state (filtered to available options)
    current_type_selection = [v for v in st.session_state.type_sel if v in type_opts]
    
    st.multiselect(
        "Type",
        options=type_opts,
        default=current_type_selection if current_type_selection else None,
        key="type_sel",
        disabled=st.session_state.master_all
    )
    st.markdown('</div>', unsafe_allow_html=True)
...
with col3:
    # Registry and Reduction/Removal in third column
    st.markdown('<div class="filter-item" style="margin-bottom: 0.05rem;">', unsafe_allow_html=True)
...
st.markdown('<div style="width:100%; text-align:center; margin-top:10px;">', unsafe_allow_html=True)
...
if st.button("Reset All Filters", on_click=reset_filters):
...
# Store filtered selections before creating widgets
filtered_country_sel = [v for v in st.session_state.country_sel if v in country_opts]
filtered_type_sel = [v for v in st.session_state.type_sel if v in type_opts]

# If master_all is on, use all available options
if st.session_state.master_all:
    filtered_country_sel = country_opts[:]
    filtered_type_sel = type_opts[:]
...
# Update session state safely before creating widgets
if 'country_sel' not in st.session_state:
    st.session_state.country_sel = filtered_country_sel
if 'type_sel' not in st.session_state:
    st.session_state.type_sel = filtered_type_sel
...
# Add Project ID filter that only affects the table
project_id_filter = st.text_input(
    "Filter by Project ID (affects table only)", 
...
# Apply Project ID filter if provided (only affects the table)
if project_id_filter.strip():
    df_display = df_display[df_display["Project ID"].astype(str).str.contains(project_id_filter.strip(), case=False, na=False)]
...
#    )
    if counts_rr.empty:
        ...
        fig_rr = px.bar(
            ...
            y="Counts",
            text="Counts",
            title="Projects by Reduction/Removal",
...
st.markdown(
    "<h4 style='text-align: center; font-size:30px;'>Credits by Reduction/Removal</h4>",
    unsafe_allow_html=True
)
...
if df_sel.empty or not needed_cols.issubset(df_sel.columns):
    ...
    fig_cr.update_traces(
        texttemplate="%{text:,}",
        hovertemplate="%{x}<br>%{legendgroup}: %{y:,} Credits<extra></extra>",
        cliponaxis=False,
    )
...
if df_sel.empty or not needed_cols.issubset(df_sel.columns):
    ...
    fig_ct.update_traces(
        texttemplate="%{text:,}",
        hovertemplate="%{x}<br>%{legendgroup}: %{y:,} Projects<extra></extra>",
        cliponaxis=False,
    )
...
#    st.caption("Tip: click a country on the map to filter. Use the button below to clear.")
    ...
    if clicks:
        ...
        if clicked:
            safe_update_session_state("country_filter", [clicked])
            st.rerun()
